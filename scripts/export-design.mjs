  ...
  return sections
    .map(({ title, start, end }) => {
      ...
      const classes = [...new Set(
        [...block.matchAll(/^\.([\w-]+)[\s{,]/gm)].map(x => x[1])
      )]
...
const main = async () => {
  ...
  md.push('> Relancer `npm run design` après toute modification des sources CSS.\n')
  ...
  // ── Tokens sémantiques ────────────────────────────────────────────────────
  md.push('\n## Tokens sémantiques\n')
  md.push('Ces tokens résolvent vers la palette et basculent automatiquement en dark mode.\n')

  const semanticGroups = [
    ['Surfaces',       k => ['--bg','--bg-soft','--bg-elev','--bg-rule','--bg-glass'].includes(k)],
    ['Texte',          k => k === '--ink' || k === '--ink-quiet' || k === '--quiet'],
    ['Bordures',       k => k === '--rule'],
    ['Couleurs accent',k => ['--teal-deep','--teal-mid','--cyan','--eyebrow'].includes(k)],
    ['Ombres & rings', k => k.startsWith('--shadow-') || k.startsWith('--ring-')],
  ]
  ...
  for (const [label, fn] of semanticGroups) {
    ...
    md.push(table(
      ['Token', 'Light', 'Dark'],
      rows.map(([k, v]) => [`\`${k}\``, `\`${v}\``, dark[k] ? `\`${dark[k]}\`` : '—']),
    ))
    md.push('')
  }
  ...
  // ── Motion ────────────────────────────────────────────────────────────────
  md.push('\n## Motion\n')
  const motion = pick(root, k => k.startsWith('--dur-') || k.startsWith('--ease-'))
  md.push(table(
    ['Token', 'Valeur'],
    motion.map(([k, v]) => [`\`${k}\``, `\`${v}\``]),
  ))
  ...
  md.push('\n## Composants CSS globaux\n')
  md.push('Classes issues de `styles.css`. Les styles scoped des composants Vue ne sont pas listés ici.\n')
  ...
  for (const { title, classes } of sections) {
    ...
    const filtered = classes.filter(c => !['root','body','where'].includes(c))
  ...
  console.log(`export-design: ${OUTPUT} écrit (${content.length} chars, ${content.split('\n').length} lignes)`)
}
